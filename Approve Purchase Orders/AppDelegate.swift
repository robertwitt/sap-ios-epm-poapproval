...
class AppDelegate: UIResponder, UIApplicationDelegate, UISplitViewControllerDelegate, OnboardingManagerDelegate, ConnectivityObserver, UNUserNotificationCenterDelegate {
    ...
    var epmrefappspoapvsrvEntities: EPMREFAPPSPOAPVSRVEntities<OnlineODataProvider>!
    ...
    func onboarded(onboardingContext: OnboardingContext, onboarding _: Bool) {
        var configurationURL = (onboardingContext.info[OnboardingInfoKey.sapcpmsSettingsParameters] as! SAPcpmsSettingsParameters).url(forDestination: "de.robertwitt.demo.epm.poapproval")
        ...
        if configurationURL == nil {
            ...
            configurationURL = URL(string: "https://hcpms-p2001023908trial.hanatrial.ondemand.com/de.robertwitt.demo.epm.poapproval")!
        }
    ...
    private func configureOData(_ urlSession: SAPURLSession, _ serviceRoot: URL) {
        let odataProvider = OnlineODataProvider(serviceName: "EPMREFAPPSPOAPVSRVEntities", serviceRoot: serviceRoot, sapURLSession: urlSession)
        // Disables version validation of the backend OData service
        // TODO: Should only be used in demo and test applications
        odataProvider.serviceOptions.checkVersion = false
        self.epmrefappspoapvsrvEntities = EPMREFAPPSPOAPVSRVEntities(provider: odataProvider)
        // To update entity force to use X-HTTP-Method header
        self.epmrefappspoapvsrvEntities.provider.networkOptions.tunneledMethods.append("MERGE")
    }
